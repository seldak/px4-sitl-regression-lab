...
`sitl_stdout.log` is cleaned of terminal redraw sequences and capped at 20 MiB. CI also rejects any individual artifact larger than 50 MiB before upload.
...
The default suite runs the baseline and low-battery scenarios:
...
## Metrics and trust gates

Horizontal tracking error is computed as:

1. **Setpoint-based error** if setpoint topics exist in the log.
2. **Geometric fallback** otherwise: distance from the actual XY track to the planned mission polyline stored in `run_metadata.json`.

This avoids depending on PX4 internal setpoint topics that can vary across versions and simulation backends.
